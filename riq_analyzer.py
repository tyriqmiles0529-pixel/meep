...
def load_priors_data() -> Optional[pd.DataFrame]:
    ...
    if not priors_dir.exists():
        if DEBUG_MODE: print("⚠️  priors_data/ directory not found - skipping priors")
        return None
    
    # Load player priors CSV
    priors_path = priors_dir / "player_priors.csv"
    if not priors_path.exists():
        if DEBUG_MODE: print("⚠️  player_priors.csv not found - skipping priors")
        return None
    
    try:
        priors = pd.read_csv(priors_path, low_memory=False)
        if DEBUG_MODE: print(f"✓ Loaded {len(priors):,} player priors ({priors.shape[1]} features)")
        return priors
    except Exception as e:
        if DEBUG_MODE: print(f"⚠️  Failed to load priors: {e}")
        return None
...
def build_player_features(
    ...
    if priors is not None and player_name:
        ...
        if not player_priors.empty:
            ...
            if DEBUG_MODE: print(f"   ✓ Matched priors for {player_name} ({len(prior_row)-1} features)")
        else:
