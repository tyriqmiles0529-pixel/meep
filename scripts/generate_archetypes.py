...
def generate_archetypes(data_path, n_clusters=8):
    ...
    latest_rows = df.sort_values(date_col).groupby(id_col).tail(1)
    
    # Filter for players with valid embeddings
    player_embs = latest_rows.dropna(subset=emb_cols)
    X = player_embs[emb_cols].values
    p_ids = player_embs[id_col].astype(str).tolist()
    ...
    player_embs['archetype_id'] = clusters
    
    # Characterize clusters by average stats
    # We need some base stats for naming
    base_stats = ['points', 'assists', 'reboundsTotal', 'minutes', 'three_pointers']
    # Check if they exist in latest_rows
    available_stats = [s for s in base_stats if s in latest_rows.columns]
    ...
    # Cast player_id to string to avoid precision issues, but keep format consistent
...
if __name__ == "__main__":
    generate_archetypes("final_feature_matrix_with_per_min_1997_onward.csv")
